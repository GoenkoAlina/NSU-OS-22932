...
void print_file_info(const char* filename) {
    ...
    if (stat(filename, &file_stat) == -1) { perror("stat"); return; }

    print_permissions(file_stat.st_mode);
    printf(" %ld", (long)file_stat.st_nlink);

    struct passwd* owner = getpwuid(file_stat.st_uid);
    if (owner != NULL) { printf(" %s", owner->pw_name); }
    else { printf(" %d", file_stat.st_uid); }

    struct group* group = getgrgid(file_stat.st_gid);
    if (group != NULL) { printf(" %s", group->gr_name); }
    else { printf(" %d", file_stat.st_gid); }

    if (S_ISREG(file_stat.st_mode)) { printf(" %ld", (long)file_stat.st_size); }
    else { printf(" "); }
